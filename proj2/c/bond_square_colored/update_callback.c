...
int init_callback(uint32_t* pixels, struct Context* cx) {
    ...
    for (size_t y = 0; y < height; ++y) {
        for (size_t x = 0; x < width; ++x) {
            ...
            ((uint8_t*)color)[(x+y*width)*4+0] = (255*(float)rand()/RAND_MAX);
            ((uint8_t*)color)[(x+y*width)*4+1] = (255*(float)rand()/RAND_MAX);
            ((uint8_t*)color)[(x+y*width)*4+2] = (255*(float)rand()/RAND_MAX);
