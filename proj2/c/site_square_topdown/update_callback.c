...
int init_callback(uint32_t* pixels, struct Context* cx) {
    ...
    cx->site_prob = malloc(sizeof(float)*width*height);
    ...
    float* site_prob = cx->site_prob;
    uint32_t* color = cx->color;
    DsufNode* uf = cx->uf;

    if (!site_prob || !uf || !color) {
        goto FAILED;
    }

    for (size_t y = 0; y < height; ++y) {
        for (size_t x = 0; x < width; ++x) {
            site_prob[x+y*width] = (float)rand()/RAND_MAX;
            uf[x+y*width].parent = &uf[x+y*width];
            color[x+y*width] = 0xff000000;
        }
    }
    
    for (size_t x = 0; x < WORLD_WIDTH; ++x) {
        color[x+0*width] = 0xffff0000;
        color[x+(WORLD_HEIGHT-1)*width] = 0xffffffff;
        site_prob[x+0*width] = 0;
        site_prob[x+(WORLD_HEIGHT-1)*width] = 0;
    }

    for (size_t y = 0; y < height; ++y) {
        for (size_t x = 0; x < BLOCKED_WIDTH; ++x) {
            site_prob[x+y*width] = 1;
        }
...
int update_callback(int pause, uint32_t* pixels_in, struct Context* cx) {
    const size_t width = WORLD_WIDTH;
    const size_t height = WORLD_HEIGHT;
    float* restrict site_prob = cx->site_prob;
    ...
    for (size_t y = 1; y < height-1; ++y) {
        for (size_t x = BLOCKED_WIDTH; x < WORLD_WIDTH - BLOCKED_WIDTH; ++x) {
            if (site_prob[x+y*width] < lvl) {
                DsufNode* restrict c = &uf[x+y*width];
                DsufNode* restrict r = &uf[(x+1)+y*width];
                DsufNode* restrict l = &uf[(x-1)+y*width];
                DsufNode* restrict t = &uf[x+(y-1)*width];
                DsufNode* restrict b = &uf[x+(y+1)*width];
               
                if (site_prob[x+y*width] < lvl) {
                    if (color[x+y*width] == 0xff000000) color[x+y*width] = 0xffffffff;
                }
                if (site_prob[x+1+y*width] < lvl) {
                    if (color[x+1+y*width] == 0xff000000) color[x+1+y*width] = 0xffffffff;
                }
                if (site_prob[x-1+y*width] < lvl) {
                    if (color[x-1+y*width] == 0xff000000) color[x-1+y*width] = 0xffffffff;
                }
                if (site_prob[x+(y-1)*width] < lvl) {
                    if (color[x+(y-1)*width] == 0xff000000) color[x+(y-1)*width] = 0xffffffff;
                }
                if (site_prob[x+(y+1)*width] < lvl) {
                    if (color[x+(y+1)*width] == 0xff000000) color[x+(y+1)*width] = 0xffffffff;
                }
                ...
                if (site_prob[x+1+y*width] < lvl) {
                    uint32_t newcolor = (color[ds_find(c)-uf] & color[ds_find(r)-uf]);
                    DsufNode* newnode = ds_union(c, r);
                    color[newnode-uf] = newcolor;
                }
                if (site_prob[x-1+y*width] < lvl) {
                    uint32_t newcolor = (color[ds_find(c)-uf] & color[ds_find(l)-uf]);
                    DsufNode* newnode = ds_union(c, l);
                    color[newnode-uf] = newcolor;
                }
                if (site_prob[x+(y-1)*width] < lvl) {
                    uint32_t newcolor = (color[ds_find(c)-uf] & color[ds_find(t)-uf]);
                    DsufNode* newnode = ds_union(c, t);
                    color[newnode-uf] = newcolor;
                }
                if (site_prob[x+(y+1)*width] < lvl) {
                    uint32_t newcolor = (color[ds_find(c)-uf] & color[ds_find(b)-uf]);
                    DsufNode* newnode = ds_union(c, b);
                    color[newnode-uf] = newcolor;
                }
            }
        }
